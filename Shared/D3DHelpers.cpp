...
ID3D11ShaderResourceView* CreateRandomTexture1DSRV(ID3D11Device* device)
{
	...
	{
		randomValues[i].x = (((float)rand() / (float)RAND_MAX)*2.0f) - 1.0f;
		randomValues[i].y = (((float)rand() / (float)RAND_MAX)*2.0f) - 1.0f;
		randomValues[i].z = (((float)rand() / (float)RAND_MAX)*2.0f) - 1.0f;
		randomValues[i].w = (((float)rand() / (float)RAND_MAX)*2.0f) - 1.0f;
	}
		...
		UINT mipFilter)
{
	...
	for(UINT i = 0; i < size; ++i)
	{
        ...
        D3DX11CreateTextureFromFileW(device, filenames[i].c_str(), 
...
void SetCBufferFloat4x4(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, const XMMATRIX &mtx)
{
	if (mapping.pData && packOffset >= 0)
	{
		XMFLOAT4X4 *pDest = reinterpret_cast<XMFLOAT4X4*>(static_cast<char *>(mapping.pData) + packOffset);
		XMStoreFloat4x4(pDest, XMMatrixTranspose(mtx));
	}
}


void SetCBufferFloat4(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, const XMFLOAT4 &vec)
{
	if (mapping.pData && packOffset >= 0)
	{
		XMFLOAT4 *pDest = reinterpret_cast<XMFLOAT4 *>(static_cast<char *>(mapping.pData) + packOffset);
		*pDest = vec;
	}
}

void SetCBufferFloat3(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, const XMFLOAT3 &vec)
{
	if (mapping.pData && packOffset >= 0)
	{
		XMFLOAT3 *pDest = reinterpret_cast<XMFLOAT3 *>(static_cast<char *>(mapping.pData) + packOffset);
		*pDest = vec;
	}
}

void SetCBufferFloat2(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, const XMFLOAT2 &vec)
{
	if (mapping.pData && packOffset >= 0)
	{
		XMFLOAT2 *pDest = reinterpret_cast<XMFLOAT2 *>(static_cast<char *>(mapping.pData) + packOffset);
		*pDest = vec;
	}
}
...
void SetCBufferArrayFloat4x4(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, int arrayIndex, const XMMATRIX &mtx)
{
	if (mapping.pData && packOffset >= 0 && arrayIndex >= 0)
	{
		XMFLOAT4X4 *pDest = reinterpret_cast<XMFLOAT4X4 *>(static_cast<char *>(mapping.pData) + packOffset) + arrayIndex;
		XMStoreFloat4x4(pDest, XMMatrixTranspose(mtx));
	}
}

void SetCBufferArrayFloat4(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, int arrayIndex, const XMFLOAT4 &vec)
{
	if (mapping.pData && packOffset >= 0 && arrayIndex >= 0)
	{
		XMFLOAT4 *pDest = reinterpret_cast<XMFLOAT4 *>(static_cast<char *>(mapping.pData) + packOffset) + arrayIndex;
		*pDest = vec;
	}
}

void SetCBufferArrayFloat3(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, int arrayIndex, const XMFLOAT3 &vec)
{
	if (mapping.pData && packOffset >= 0 && arrayIndex >= 0)
	{
		...
		XMFLOAT3 *pDest = reinterpret_cast<XMFLOAT3 *>(static_cast<char *>(mapping.pData) + packOffset + arrayIndex * 4 * sizeof(float));
		*pDest = vec;
	}
}

void SetCBufferArrayFloat2(const D3D11_MAPPED_SUBRESOURCE &mapping, int packOffset, int arrayIndex, const XMFLOAT2 &vec)
{
	if (mapping.pData && packOffset >= 0 && arrayIndex >= 0)
	{
		// NOTE packing rules for HLSL constants: http://msdn.microsoft.com/en-us/library/bb509632(v=vs.85).aspx
		...
		// Executive summary: "Arrays are not packed in HLSL by default ...
		// every element in an array is stored in a four-component vector".
		XMFLOAT2 *pDest = reinterpret_cast<XMFLOAT2 *>(static_cast<char *>(mapping.pData) + packOffset + arrayIndex * 4 * sizeof(float));
		*pDest = vec;
	}
}
